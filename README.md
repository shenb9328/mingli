...
```
[原始典籍底本库 (clean_texts/)] ─> 8 部传世经典（三命通会、渊海子平、滴天髓、滴天髓阐微、滴天髓补注、穷通宝鉴、子平真诠、神峰通考）
       │
       ▼ (E0~E5 文献证据逐字溯源，杜绝伪造)
┌───────────────────────────────────────────────────────────────────────────┐
│ 1. 底层核心计算裁决层 (data/core_rules.json + data/classical_texts.json)    │
│    • 全库收录 1,398 条逐字古籍原文证据 + 1,002 条核心判定规则              │
...
项目在根目录提供经过严格校勘与去噪的 8 部命理传世经典文本（UTF-8 纯文本，字数逾 80 万字），供学术研究、文本检索与模型训练使用：
